    ...
    def generate_summary_statistics(results: dict) -> dict:
        ...
        for host, data in results.items():
            ...
            for service in services:
                ...
                if service_name in stats['service_distribution']:
                    stats['service_distribution'][service_name] += 1
                else:
                    stats['service_distribution'][service_name] = 1
        
        # 가장 많이 발견된 포트 (상위 10개)
        sorted_ports = sorted(port_counts.items(), key=lambda x: x[1], reverse=True)
        stats['most_common_ports'] = dict(sorted_ports[:10])
        
        return stats
    
    @staticmethod
    def format_risk_matrix(findings: List[dict]) -> dict:
        """위험도 매트릭스 형식으로 포맷"""
        matrix = {
            'critical': [],
            'high': [],
        ...
        for finding in findings:
            ...
            if severity in matrix:
                matrix[severity].append(finding)
